...
function(a){jQuery("body").addClass("lp-"+a)};a.onLoadComplete=function(){var c="undefined"==typeof lp_close_btn&&"additionalSeconds"in a.options&&jQuery.isNumeric(a.options.additionalSeconds)?parseInt(a.options.additionalSeconds):0,b=function(){jQuery("#loading_page_codeBlock,.lp-close-screen").remove();a.graphicAction("complete",function(){a.ApplyAnimationToElement(a.options.pageEffect);a.options.onComplete();jQuery(document).trigger("loadingScreenCompleted")})};c?setTimeout(function(){b()},1E3*
c):b()};a.setLoadImgEvents=function(){for(var c=jQuery(window).height(),d=0,f=b.length;d<f;d++)if(!a.imgLoaded(b[d])){var e=jQuery(b[d]);if(a.options.deepSearch||e.offset().top<c)h++,e.on("load",function(){a.completeImageLoading()})}0==h&&this.completeImageLoading()};a.completeImageLoading=function(){f++;a.graphicAction("set",h?Math.min(f/h*100,100):100);h<=f&&!(1*a.options.removeInOnLoad)&&a.destroyLoader()};a.destroyLoader=function(){if(a.options.loadingScreen&&!g){a.displayBody();var b=Math.min(h?
f/h*100:0,100),d=function(){a.graphicAction("set",b);if(100==b)a.onLoadComplete();else b=Math.min(b+5,100),setTimeout(d,0)};g=!0;a.loadOriginalImg();d()}};a.loadingPage=function(){a.displayBody();b=jQuery("body").find("img:not(.lp-lazy-applied)");b.length&&a.setLoadImgEvents()};a.displayScreen=function(){jQuery(document).trigger("loadingScreenShow");if(a.options.closeBtn){var b=jQuery('<span class="lp-close-screen">X</span>');b.click(function(){lp_close_btn=!0;if(g)a.onLoadComplete();else a.destroyLoader()});
jQuery("html").append(b)}a.options.codeblock&&a.options.codeblock.length&&jQuery("html").append(a.options.codeblock);"undefined"!=typeof a.graphics&&"undefined"!=typeof a.graphics[a.options.graphic]&&(a.graphics[a.options.graphic].create(a.options),b=document.getElementById("loading_page_codeBlock"))&&(b.style.display="block")};a.options="undefined"!=typeof loading_page_settings?a.extend(e,loading_page_settings):e;a.options.text*=1;a.options.loadingScreen*=1;a.options.loadingScreen&&a.validateScreenSize()?
a.displayScreen():(a.displayBody(),a.options.loadingScreen=0)})();jQuery(function(){cp_loadingpage.options.loadingScreen&&cp_loadingpage.loadingPage()});jQuery(window).on("load",function(){cp_loadingpage.loadOriginalImg();cp_loadingpage.destroyLoader()});
